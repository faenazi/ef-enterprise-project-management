...
6. ملفات تصميم الحل وUI/UX والجودة والإصدارات عند إنشائها
...
- لا يجوز إعادة إدخال أي تكامل مباشر خارج النطاق المعتمد دون قرار نطاق موثق وتحديث جميع الوثائق المتأثرة.

## قرار نطاق التكامل المعتمد

تم اعتماد نطاق التكامل المباشر للنظام على ثلاثة أنظمة فقط:

| التكامل المعتمد | الغرض المعتمد | تفاصيل ما زالت مطلوبة |
|---|---|---|
| Oracle Fusion Cloud ERP | التكامل مع بيانات ومعاملات ERP ذات العلاقة بالمشاريع | الوحدات والكائنات والحقول وواجهات API والاتجاه والتواتر والتسوية ومعالجة الفشل والملكية |
| Microsoft Entra ID | المصادقة المؤسسية وSingle Sign-On | إعداد التطبيق والبروتوكول والـClaims وربط الأدوار والمجموعات وMFA والجلسات والوصول الخارجي |
| Microsoft Exchange | البريد والتقويم المؤسسي | الإشعارات والبريد والاجتماعات ودعوات التقويم والصلاحيات والقوالب والتتبع وحدود الخدمة |

أي تكامل مباشر آخر ورد في مسودات سابقة، بما في ذلك OpenText ECM أو أنظمة الموارد البشرية أو التحليلات أو المراسلات أو الاستراتيجية أو المخاطر أو ITSM أو أدوات التنفيذ، أصبح خارج النطاق الحالي ويجب حذفه أو تعليمه `Out of Scope` أو تحويله إلى سلوك غير متكامل عند الحاجة.
...
| التكامل | `INT` | System Requirements — يحتاج مواءمة مع التكاملات الثلاثة المعتمدة |
...
| متطلبات الأعمال | `BR-001` إلى `BR-132` | 132 من 132 | Partial | جميعها مغطاة ضمن نطاقات القدرات، لكنها ما زالت Draft وتحتاج مواءمة نطاق التكامل. |
| قواعد الأعمال المرشحة | `CBR-001` إلى `CBR-020` | 20 من 20 كمرشحات | Gap | لم تعتمد كقواعد أعمال رسمية ولم تربط تفصيليًا بكل FR وBI. |
| المتطلبات الوظيفية | `FR-001` إلى `FR-332` | 332 من 332 | Partial | جميعها مغطاة ضمن 24 مجموعة قدرة؛ الربط التفصيلي بعناصر التسليم مطلوب، وتحتاج متطلبات التكامل إلى مواءمة. |
...
| عناصر سجل الأعمال | `BI-001` إلى `BI-214` | 214 من 214 | Partial | جميع العناصر ضمن مجموعات القدرات، وجميعها Not Ready وNot Started، وتحتاج عناصر التكامل إلى مراجعة. |
...
| نطاق التكامل المباشر | 3 أنظمة معتمدة | 3 من 3 محددة اسميًا | Partial | النطاق مؤكد، لكن العقود والحقول والاتجاه والتواتر والأمن والتسوية لم تصمم بعد. |
...
| OBJ-005 | BI-087، BI-097، BI-180، BI-190 | التكامل المحكوم مع Oracle Fusion Cloud ERP وMicrosoft Entra ID وMicrosoft Exchange وحماية حدود مصادر الحقيقة. | Partial |
...
| أمن الهوية والتطبيق والتكامل | NFR-015–NFR-023 | BI-180–BI-200 | تصميم الأمن وEntra ID والجلسات والتشفير والأسرار وواجهات التكامل الثلاثة | فحص ثغرات واختبار اختراق واختبارات وصول وتكامل | Partial |
...
| سلامة البيانات وحداثتها وموثوقية التكامل والتزامن والأرشفة | NFR-043–NFR-048 | BI-144–BI-152، BI-190–BI-200 | نموذج البيانات وعقود Oracle وEntra وExchange والتسوية والتزامن والأرشفة | اختبارات جودة ومطابقة وIdempotency وتعارض واسترجاع | Partial |
...
| التكاملات | النطاق المعتمد: Oracle Fusion Cloud ERP، Microsoft Entra ID، Microsoft Exchange | النطاق الاسمي مؤكد، بينما سجل `INT-001–INT-014` في SRS ما زال يمثل مسودة سابقة متعارضة | مواءمة سجل INT وإبقاء ثلاثة تكاملات فقط، مع عدم إعادة استخدام أي معرف محذوف دون تسجيل حالة `Out of Scope` أو بديل موثق. |
...
| TG-007 | تصميم البيانات وAPI للتكاملات الثلاثة المعتمدة غير موجود. | لا تزال كائنات Oracle وClaims الخاصة بـEntra ونطاق Exchange والعقود والتسوية والفشل غير محسومة. | إعداد Data/API/Integration Design للتكاملات الثلاثة فقط وربط DATA وINT وFR وNFR. | Data and Integration Owners — TBD | Open |
...
| TG-014 | الملاك الرسميون والسياسات ومصفوفات الصلاحيات والتفاصيل الداخلية للأنظمة المصدر غير مؤكدة. | تبقى المتطلبات والمسارات والربط في حالة Draft رغم تأكيد أسماء أنظمة التكامل الثلاثة. | حسم PIQ وBOQ وSOQ ذات الصلة وتحديث جميع الوثائق. | Environment Fund — TBD | Open |
...
| TG-016 | تحتوي Project Overview وBRD وSRS وDelivery Backlog على إشارات أو عناصر تكامل خارج النطاق المعتمد. | تعارض وثائقي قد يؤدي إلى تصميم أو تقدير أو تنفيذ تكاملات غير مطلوبة. | مواءمة الملفات الأربعة وإبقاء Oracle Fusion Cloud ERP وMicrosoft Entra ID وMicrosoft Exchange فقط، مع تسجيل العناصر الملغاة تاريخيًا. | Business/System Analysis — TBD | Open |
...
8. التحقق من عدم وجود تكامل مباشر خارج الأنظمة الثلاثة المعتمدة.
9. تشغيل Markdown Lint والتحقق من هيكل الوثائق والمراجع.
...
- 214 سجل Backlog؛
- 24 معيار قبول أوليًا للـEpics؛
- 12 ملف قبول مشتركًا؛ و
- ثلاثة تكاملات مباشرة معتمدة اسميًا.

مع ذلك، تبقى الوثيقة `Draft` ولا تعتبر جاهزة للتنفيذ؛ لأن التتبع التفصيلي لكل `FR` و`NFR` إلى عنصر تسليم وتصميم واختبار وإصدار لم يكتمل، ولأن ملفات المتطلبات السابقة لم تُواءم بعد مع قرار التكامل، ولأن العقود والحقول والاتجاهات والمقاييس والتصاميم والاختبارات والإصدارات ما زالت غير معتمدة أو لم تبدأ.
...
| 0.3 | 2026-07-18 | تحليل أعمال وأنظمة بمساعدة الذكاء الاصطناعي | تثبيت نطاق التكامل المباشر على Oracle Fusion Cloud ERP وMicrosoft Entra ID وMicrosoft Exchange فقط، وتسجيل فجوة مواءمة الوثائق السابقة. |
